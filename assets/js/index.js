...
function getNews(news) {
  ...
  sectionNews.append(articleNews);
  articleNews.classList.add("articleNews");

  const listNews = document.createElement("ul");
  articleNews.append(listNews);
  listNews.classList.add("listNews");

  const titleNews = createNewElement("li", news.title, "titleNews");
  const headerBgSrcNews = createNewElement(
    "img",
    news.headerBgSrc,
    "headerBgSrcNews"
  );
  const categoryNews = createNewElement("li", news.category, "categoryNews");
  const bodyNews = createNewElement("li", news.body, "bodyNews");
  const dateNews = createNewElement("li", news.date, "dateNews");

  // const titleNews = document.createElement("li");
  // titleNews.textContent = news[0].title;
  // titleNews.classList.add("titleNews");
  // listNews.append(titleNews);

  // const headerBgSrcNews = document.createElement("img");
  // headerBgSrcNews.src = news[0].headerBgSrc;
  // headerBgSrcNews.classList.add("headerBgSrcNews");
  // listNews.append(headerBgSrcNews);

  // const categoryNews = document.createElement("li");
  // categoryNews.textContent = news[0].category;
  // categoryNews.classList.add("categoryNews");
  // listNews.append(categoryNews);

  // const bodyNews = document.createElement("li");
  // bodyNews.textContent = news[0].body;
  // bodyNews.classList.add("bodyNews");
  // listNews.append(bodyNews);

  // const dateNews = document.createElement("li");
  // dateNews.textContent = news[0].date;
  // dateNews.classList.add("dateNews");
  // listNews.append(dateNews);
